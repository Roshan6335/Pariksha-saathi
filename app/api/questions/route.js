...
export async function POST(request) {
  try {
    ...
    let { subject, chapter, count = 5, alreadyAsked = [], questionType = "mixed", difficulty = "mixed" } = body;

    subject = capString(subject, 50);
    chapter = capString(chapter, 200);
    alreadyAsked = Array.isArray(alreadyAsked) ? alreadyAsked.slice(-25).map((s) => capString(s, 300)) : [];
    ...
    const difficultyInstructions = {
      easy: "Keep these at an easy, confidence-building level — direct recall and straightforward application, the kind of questions a student who has just started revising this chapter should be able to answer.",
      medium: "Keep these at a solid medium level — typical CBSE board exam difficulty, requiring proper understanding but not the trickiest edge cases.",
      hard: "Make these genuinely challenging — the level that separates top scorers, with multi-step reasoning, less obvious applications, or commonly-confused concepts.",
      mixed: "Use a natural mix of easy, medium, and hard questions across the set, like a real exam paper would.",
    };
...
For case-study ones, start "q" with a short 2-4 sentence real-world scenario, then the actual question.`,
    ...
    const systemPrompt = `You are a CBSE Class 10 ${subject} question paper setter with deep knowledge of the NCERT syllabus. Generate exactly ${n} NEW multiple choice questions strictly scoped to the given chapter only — do not pull in content from other chapters.

${difficultyInstructions[difficulty] || difficultyInstructions.mixed}
