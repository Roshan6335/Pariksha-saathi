...
function Flashcards() {
  ...
  async function generate() {
    ...
    try {
      ...
      const alreadyMade = JSON.parse(localStorage.getItem(madeKey) || "[]");
      ...
      localStorage.setItem(madeKey, JSON.stringify(updated));
  ...
  function nextCard(knewIt) {
    ...
    else setReviewQueue((q) => [...q, cards[index]]);
...
function StudyPlanner() {
  ...
  useEffect(() => {
    try {
      const savedPlan = JSON.parse(localStorage.getItem(plannerStorageKey()) || "null");
      const savedDone = JSON.parse(localStorage.getItem(plannerDoneKey()) || "{}");
  ...
  async function generate() {
    ...
    try {
      ...
      setPlan(data.plan);
      setDone({});
      localStorage.setItem(plannerStorageKey(), JSON.stringify(data.plan));
      localStorage.setItem(plannerDoneKey(), JSON.stringify({}));
    } catch (e) {
      setError(e.message);
    }
    setLoading(false);
  ...
  function toggleDone(key) {
    ...
    setDone(updated);
    localStorage.setItem(plannerDoneKey(), JSON.stringify(updated));
  }
