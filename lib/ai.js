...
function buildUserContent(userText, imageDataUrls) {
  if (!imageDataUrls || !imageDataUrls.length) return userText;
  return [
 ...
 */
export async function callAI({ systemPrompt, userText, imageDataUrl, imageDataUrls, maxTokens }) {
  ...
  const images = imageDataUrls && imageDataUrls.length ? imageDataUrls : imageDataUrl ? [imageDataUrl] : [];
